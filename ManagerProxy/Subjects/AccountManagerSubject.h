...
public:
	...
	virtual std::list<std::weak_ptr<MerchantAccount>> getMerchantList() = 0;
	...
	virtual std::weak_ptr<MerchantAccount> getMerchant(const unsigned long id) = 0;
	...
	virtual std::weak_ptr<CustomerAccount> getCustomer(const unsigned long id) = 0;
