...
public:
	...
	unsigned long getAccountIdIfPasswordRight(std::string account, std::string password) override;
	/// @override
//	void registerAuthenticationCarrier(std::shared_ptr<AuthenticationCarrier> carrier) override;
	...
	MerchantAccount *getMerchant(const unsigned long id) override;
//	std::weak_ptr<MerchantAccount> getMerchant(const std::string &account) override;
	CustomerAccount *getCustomer(const unsigned long id) override;
