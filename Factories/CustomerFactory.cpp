...
{
	auto accountInfo = DATA_SOURCE_FROM::getInstance().loadCustomer(id);
...
{
	unsigned long id = DATA_SOURCE_FROM::getInstance().checkCustomerPasswordAndGetId(account, password);
...
{
	DATA_SOURCE_FROM::getInstance().updateCustomerAccountPassword(account, password);
}