...
{
	auto accountInfo = DataSourceFrom::getInstance().loadCustomer(id);
...
{
	unsigned long id = DataSourceFrom::getInstance().checkCustomerPasswordAndGetId(account, password);
...
{
	DataSourceFrom::getInstance().createCustomerAccount(account, password);
}

void CustomerFactory::updateAccountSpecific(std::string account, std::string password)
{
	DataSourceFrom::getInstance().updateCustomerAccountPassword(account, password);
}